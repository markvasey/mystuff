        ...
        try (YoloPoseDetector detector = new YoloPoseDetector();
            ...
            while (true) {
                ...
                for (TrackedPerson p : trackedPeople) {
                    ...
                    if (p.isTonicWarning()) {
                        System.out.printf("[Frame %d] WARNING: TONIC PHASE detected! Peak Frequency: %.2f Hz\n", frameIdx, p.getPeakFrequencyHz());
                    }
                    if (p.isSeizureDetected()) {
                        seizureCount++;
                        System.out.printf("[Frame %d] SEIZURE ALARM (CONFIRMED CLONIC ALERT)! Peak Frequency: %.2f Hz\n", frameIdx, p.getPeakFrequencyHz());
                    }
                }
            }
