...
public class SemanticProbe {
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            ...
            double dist = calculateEuclideanDistance(weights, id1, id2);
    ...
    private static double calculateEuclideanDistance(Matrix weights, int id1, int id2) {
        int dim = weights.getCols();
        double sumSq = 0;
        for (int i = 0; i < dim; i++) {
            double diff = weights.get(id1, i) - weights.get(id2, i);
            sumSq += diff * diff;
        }
