...
public class TrackedPerson {
    ...
    private static final int HISTORY_SIZE = 40; // Approx 2 seconds at 20 fps
    private static final double MIN_SEIZURE_MOTION = 0.1; // Threshold to ignore sensor noise
    private static final double PEAK_SENSITIVITY = 1.35; // Multiplier above mean to detect a rhythmic peak
    private static final int MIN_SEIZURE_PEAKS = 1; // Minimum rhythmic peaks in window to flag seizure
    private static final int MAX_SEIZURE_PEAKS = 5; // Maximum rhythmic peaks in window to flag seizure
    ...
    private void analyzeSeizure() {
        ...
        for (Double val : motionHistory) {
            if (val > mean * PEAK_SENSITIVITY && !above) {
                peaks++;
                above = true;
        ...
        // Seizure rhythm check
        // If 60 frames = 2 seconds, then 4 to 12 peaks = 2-6Hz
        seizureDetected = (peaks >= MIN_SEIZURE_PEAKS && peaks <= MAX_SEIZURE_PEAKS);
